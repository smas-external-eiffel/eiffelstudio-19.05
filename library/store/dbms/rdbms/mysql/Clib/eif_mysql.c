 ...
 * Date: $Date: 2014-12-16 09:09:46 +0000 (Tue, 16 Dec 2014) $
 *
...
/*
 * Function:	long eif_mysql_boolean_data(MYSQL_ROW row_ptr, int ind)
 * Description:	Get the boolean at field ind in row_ptr
 * Arguments:	MYSQL_ROW row_ptr:	Pointer to the current row
 *				int ind:			Column index
 * Returns:		The boolean in that field
 */
EIF_BOOLEAN eif_mysql_boolean_data(MYSQL_ROW row_ptr, int ind)
{
	if(*row_ptr[ind - 1] == 1)
		return EIF_TRUE;
	else
