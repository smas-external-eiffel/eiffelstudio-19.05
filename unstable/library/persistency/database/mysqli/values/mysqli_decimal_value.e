...
feature -- Access (Date and Time Types)
	...
	as_datetime: DATE_TIME
		...
		do
			create Result.make_now
			raise ("MYSQLI_DECIMAL_VALUE.as_datetime: current 'value' is not representable as a datetime.")
		end

	as_year: INTEGER
			-- This value as a INTEGER
		do
			raise ("MYSQLI_DECIMAL_VALUE.as_year: current 'value' is not representable as a year.")
		end
...
feature -- Access (Set Type)
	...
	as_set: SET [STRING]
		...
		do
			...
			Result := l_linked_set
			raise ("MYSQLI_DECIMAL_VALUE.as_set: current 'value' is not representable as a set.")
		end
